...
func (c *Client) AddRCPC() (err error) {
	if c.RCPC != "" {
		color.Green("Current RCPC token: %v", c.RCPC)
	}
	color.Cyan("Configure your RCPC token")
	color.Cyan("Note: You will have to retrieve your RCPC token manually")
	fmt.Printf("RCPC token: ")
	RCtoken := util.ScanlineTrim()
	if RCtoken != "" {
		color.Green("New RCPC token: %v", RCtoken)
		c.RCPC = RCtoken
		return c.save()
	}
	return c.save()
